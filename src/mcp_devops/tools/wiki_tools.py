...
def _get_api_path(wiki_id: str) -> str:
    return f"{WIKI_API_PATH}/{wiki_id}"


def _get_wiki_page_details(wiki_id: str, page_id: int, include_content: bool) -> dict:
    url = (
        f"{devops_api_url}/{_get_api_path(wiki_id)}"
        f"/pages/{page_id}?includeContent={str(include_content).lower()}&api-version={DEVOPS_API_VERSION}"
    )
    return devops_api_get(url)


def _get_wiki_info(wiki_id: str) -> dict:
    if wiki_id not in WIKI_INFO_CACHE:
        WIKI_INFO_CACHE[wiki_id] = devops_api_get(
            f"{devops_api_url}/{_get_api_path(wiki_id)}?api-version={DEVOPS_API_VERSION}"
        )
...
@mcp.tool(
    name="devops_wiki_page_get",
    description=(
        "Retrieves metadata (path, isParentPage) and/or content of a wiki page. "
        "The wiki_id and page_id are found in the browser URL: "
        "https://.../_wiki/wikis/{wiki_id}/{page_id}/..."
    ),
)
def get_wiki_page(
    wiki_id: Annotated[str, "Wiki identifier (name or ID). Found in the page URL: .../_wiki/wikis/{wiki_id}/..."],
    page_id: Annotated[int, "Wiki page ID. Found in the page URL: .../_wiki/wikis/{wiki_id}/{page_id}/..."],
    include_content: bool = False,
) -> object:
    """Get wiki page metadata and content by wiki_id and page_id."""
    page = _get_wiki_page_details(wiki_id, page_id, include_content)
    result = {
        "path": page.get("path"),
...
def create_wiki_page(
    ...
    base = f"{devops_api_url}/{_get_api_path(wiki_id)}"
    ...
    wiki_info = _get_wiki_info(wiki_id)
